...
public:
  ...
  ImuProcess(std::shared_ptr<rclcpp::Node> node);
  ~ImuProcess();
...
ImuProcess::ImuProcess(std::shared_ptr<rclcpp::Node> node)
    : b_first_frame_(true), imu_need_init_(true), gravity_align_(false), node_(node)
{
  imu_en = true;
  init_iter_num = 1;
...
{
  RCLCPP_WARN(node_->get_logger(), "Reset ImuProcess");
  mean_acc = V3D(0, 0, -1.0);
  mean_gyr = V3D(0, 0, 0);
  imu_need_init_ = true;
  init_iter_num = 1;
...
void ImuProcess::IMU_init(const MeasureGroup &meas, int &N)
{
  ...
  RCLCPP_INFO(node_->get_logger(), "IMU Initializing: %.1f %%", double(N) / MAX_INI_COUNT * 100.0);
  V3D cur_acc, cur_gyr;
...
void ImuProcess::Process(const MeasureGroup &meas, PointCloudXYZI::Ptr cur_pcl_un_)
{
  if (imu_en)
  {
    ...
    RCLCPP_ASSERT(node_->get_logger(), meas.lidar != nullptr);
    ...
    if (imu_need_init_)
    {
      ...
      {
        RCLCPP_INFO(node_->get_logger(), "IMU Initializing: %.1f %%", 100.0);
