...
Preprocess::Preprocess(std::shared_ptr<rclcpp::Node> node)
  : lidar_type(AVIA), blind(0.01), point_filter_num(1), node_(node)
{
  inf_bound = 10;
  N_SCANS   = 6;
  SCAN_RATE = 10;
  group_size = 8;
  disA = 0.01;
  disB = 0.1; // Corrected from the code where disA was assigned twice
  p2l_ratio = 225;
  limit_maxmid = 6.25;
  limit_midmin = 6.25;
  limit_maxmin = 3.24;
  ...
  jump_up_limit = cos(jump_up_limit / 180.0 * M_PI);
  jump_down_limit = cos(jump_down_limit / 180.0 * M_PI);
  cos160 = cos(cos160 / 180.0 * M_PI);
  smallp_intersect = cos(smallp_intersect / 180.0 * M_PI);
}
...
void Preprocess::process(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg, PointCloudXYZI::Ptr &pcl_out)
{
  ...
  switch (lidar_type)
  {
    ...
    case UNILIDAR:
      ...
      break;
    default:
      RCLCPP_ERROR(node_->get_logger(), "Error LiDAR Type");
      break;
  }
...
void Preprocess::oust64_handler(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg)
{
  ...
  pl_corn.clear();
  pl_full.clear();
  pl_full.header = pcl_conversions::toPCL(msg->header);
...
void Preprocess::give_feature(PointCloudXYZI &pl, std::vector<orgtype> &types)
{
  int plsize = pl.size();
  if (plsize == 0)
  {
    RCLCPP_WARN(node_->get_logger(), "PointCloud size is zero in give_feature");
    return;
  }
  uint head = 0;

  while (types[head].range < blind && head < types.size())
  {
    head++;
  }

  int plsize2 = (plsize > group_size) ? (plsize - group_size) : 0;

  Eigen::Vector3d curr_direct(Eigen::Vector3d::Zero());
  Eigen::Vector3d last_direct(Eigen::Vector3d::Zero());

  uint i_nex = 0, i2;
  uint last_i = 0, last_i_nex = 0;
  int last_state = 0;
  int plane_type;
